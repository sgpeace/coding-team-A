...
final_capitals = []
transaction_counts = []  # 거래 횟수를 저장할 리스트 초기화

# 초기 자본금 설정
initial_capital = 1000000  # 100만원

# 수수료 및 세금 비율 설정
buy_fee_rate = 0.00364 / 100  # 매수 수수료
sell_fee_rate = 0.00364 / 100  # 매도 수수료
tax_rate = 0.18 / 100  # 매도 시 세금

# 각 total_score 범위에 대해 최종 자본금과 거래 횟수를 계산
for lower, upper in score_ranges:
    ...
    for daily_return in result_df['Daily Return']:
        # 매수 시 수수료 적용
        capital *= (1 - buy_fee_rate)
        
        # 수익률 적용
        capital *= (1 + daily_return * 0.01)
        
        # 매도 시 수수료 및 세금 적용
        capital *= (1 - sell_fee_rate - tax_rate)
...
result_table = pd.DataFrame({
    ...
    'Transaction Count': transaction_counts  # 거래 횟수 추가
})
