...
# 1. 일별 수익률 불러오기
df = pd.read_excel('OC_data.xlsx')  # 파일 경로에 맞게 변경
...
for lower, upper in score_ranges:
    ...
    # 일별 수익률을 적용하여 자본금 계산
    hanmi_capital = initial_capital
    kospi_capital = 0  # Kospi 숏 포지션의 초기 자본금은 별도로 설정

    for index, row in result_df.iterrows():
        hanmi_return = row['Daily Return']
        kospi_return = row['Kospi']  # E열의 Kospi 선물 수익률

        # 한미약품 수익률 적용 (단리)
        hanmi_capital += initial_capital * (hanmi_return * 0.01)

        # Kospi 선물 수익률 적용 (베타 고려, 단리)
        kospi_capital += initial_capital * beta * (-kospi_return)

    # 손익 계산
    hanmi_profit = hanmi_capital - initial_capital
    kospi_profit = kospi_capital
    total_profit = hanmi_profit + kospi_profit

    # 결과 저장
    hanmi_profits.append(hanmi_profit)
    kospi_profits.append(kospi_profit)
    final_profits.append(total_profit)

# 결과를 데이터프레임으로 정리
result_table = pd.DataFrame({
    'Score Range (Lower, Upper)': [f"{lower} - {upper}" for lower, upper in score_ranges],
    'Total Profit (KRW)': final_profits,
    'Hanmi Profit (KRW)': hanmi_profits,
    'Kospi Profit (KRW)': kospi_profits,
    'Total Profit (%)': [round(profit / initial_capital * 100, 2) for profit in final_profits]
})
...
# 필요한 경우 엑셀 파일로 저장
result_table.to_excel('final_capital_market_neutral.xlsx', index=False)