...
high_score_dates = df1[(df1['total_score'] >= 0.6) & (df1['total_score'] <= 0.8)]['Date']
...
# 거래 횟수를 계산하는 부분
transaction_count = len(transactions)  # 예시로 transactions 리스트의 길이를 사용

# 엑셀 파일에 기록하는 부분
worksheet.write('D1', 'tx')  # D열의 첫 번째 셀에 'tx'라는 제목 추가
worksheet.write('D2', transaction_count)  # D열의 두 번째 셀에 거래 횟수 기록